...
Original file is located at
    https://colab.research.google.com/drive/1s_ReoSI5Dd5j82Wizd3-OMM9oZrfPG7Q
"""
    ...
    for i in range(0, ARRAY_LENGTH):
      for j in range(0, ARRAY_LENGTH):
        if numData[i][j] < 0:
          ...
          retlist.append(numData[i][j]%100)
        else:
          retlist.append(numData[i][j])
    retData = [[retlist[(i*ARRAY_LENGTH)+j] for j in range(ARRAY_LENGTH)] for i in range(ARRAY_LENGTH)]
    ###########   <-------------- 여기까지 코딩 (1)
...
def getMaxSum(numData): # 요구 사항에 맞춰 처리된 2차원 정수 배열
    ...
    for i in range(ARRAY_LENGTH-1):
      for j in range(ARRAY_LENGTH-1):
        instsum = numData[i][j] + numData[i+1][j] + numData[i][j+1] + numData[i+1][j+1]
        if instsum > maxSum:
          maxSum = instsum
